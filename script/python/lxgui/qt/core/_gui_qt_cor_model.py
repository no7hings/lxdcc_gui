...
class GuiTabItemStackModel(object):
    ITEM_CLS = GuiTabItemModel
...
class GuiGridLayoutModel(object):
    def __init__(self):
        self.__count = 0
    ...
    def get_item_column_at(self, index):
        return GuiGrid.get_column_at(index, self.__column_count)

    def get_item_row_at(self, index):
        return GuiGrid.get_row_at(index, self.__column_count)
    ...
    def update(self):
        ...
        if self.__count:
            m_l, m_t, m_r, m_b = self._item_margins
            item_w, item_h = self.__item_w+m_l+m_r, self.__item_h+m_t+m_b
            self.__column_count = GuiGrid.to_column_count(self.__w, item_w)
            self.__column_count = min(self.__column_count, self.__count)
            self.__row_count = GuiGrid.get_row_count(self.__count, self.__column_count)
            self.__abs_w, self.__abs_h = GuiGrid.get_abs_size(item_w, item_h, self.__column_count, self.__row_count)
    ...
    def get_coord_loc(self, x, y):
        column = GuiGrid.get_column_loc(x, self.__item_w)
        row = GuiGrid.get_row_loc(y, self.__item_h)
        return column, row

    def get_index_between(self, column, row):
        return GuiGrid.get_index_between(column, row, self.__column_count)

    def get_index_loc(self, x, y):
        pass


class GuiVLayoutModel(object):
    def __init__(self):
        self.__x, self.__y = 0, 0
...
class GuiLayoutItemModel(object):
...
class GuiLayoutItemStackModel(object):
    ITEM_CLS = GuiLayoutItemModel

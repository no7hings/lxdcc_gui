...
class _GuiPageQuery(object):
    ...
    def restore(self):
        ...
        self.__all_page_keys_department = kit_core.KitDesktopHook.find_all_group_keys_at(
            kit_core.KitDesktopHook.PageKey.Department
        )
        self.__page_keys_department = []

        self.__all_page_keys_user = kit_core.KitDesktopHook.find_all_group_keys_at(
            kit_core.KitDesktopHook.PageKey.User
        )
        self.__page_keys_user = []

        self.__all_page_keys = self.__all_page_keys_default \
                              +self.__all_page_keys_department \
                              +self.__all_page_keys_user

        self.__page_keys = self.__all_page_keys_default \
                          +[kit_core.KitDesktopHook.get_current_user_group_key()]

    def key_is_user(self, key):
        return key in self.__all_page_keys_user

    def push_to_using(self, key):
        if key in self.__all_page_keys_default:
            self.__page_keys_default.append(key)
        elif key in self.__all_page_keys_department:
            self.__page_keys_department.append(key)
        elif key in self.__all_page_keys_user:
            self.__page_keys_user.append(key)

    def pull_from_using(self, key):
        if key in self.__page_keys_default:
            self.__page_keys_default.remove(key)
        elif key in self.__page_keys_department:
            self.__page_keys_department.remove(key)
        elif key in self.__page_keys_user:
            self.__page_keys_user.remove(key)
...
class AbsToolKitForDesktop(prx_widgets.PrxSessionWindow):
    ...
    def gui_add_customize_for_page(self, page_name, switch_to=False):
        ...
        hook_keys = kit_core.KitDesktopHook.find_all_keys_at(page_name)
        for i_hook_key in hook_keys:
            self.gui_add_one_for_customize(i_hook_key, page_name)

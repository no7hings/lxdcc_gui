    ...
    def __build_data_fnc_(self):
        ...
        with bsc_core.LogProcessContext.create(maximum=count, label='gui-add for geometry-comparer result') as g_p:
            for i_path_src, i_path_tgt, i_description in self._comparer_results:
                ...
                i_status = utl_core.DccDialog.ValidationStatus.Correct
                for j_key in i_keys:
                    if j_key in self.ERROR_STATUS:
                        i_status = utl_core.DccDialog.ValidationStatus.Error
                        break
                    elif j_key in self.WARNING_STATUS:
                        i_status = utl_core.DccDialog.ValidationStatus.Warning
                        break

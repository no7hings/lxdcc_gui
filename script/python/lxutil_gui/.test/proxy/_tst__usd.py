...
class W(prx_widgets.PrxBaseWindow):
    def __init__(self, *args, **kwargs):
        ...
        self._dtb_opt = dtb_objects.DtbResourceLibraryOpt(
            ...
            bsc_core.RscConfigure.get_yaml('database/library/resource-{}'.format(self._category_group))
        )
    ...
    def build_lib_resource(self, resource_path):
        def cache_fnc_():
            bsc_core.Log.test_start('build stage')
            ...
            bsc_core.Log.test_end('build stage')
            return []

        def build_fnc_():
            bsc_core.Log.test_start('refresh stage')
            self._usd_stage_view.refresh_usd_view_draw()
            bsc_core.Log.test_end('refresh stage')

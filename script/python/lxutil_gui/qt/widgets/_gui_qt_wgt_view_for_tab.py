...
from lxutil_gui.qt.widgets import _gui_qt_wgt_utility, _gui_qt_wgt_button, _gui_qt_wgt_layer_stack
...
class QtTabView(
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._tab_add_button = _gui_qt_wgt_button.QtIconMenuButton(self)
        self._tab_add_button.hide()
        self._tab_add_button._set_icon_file_path_(
            gui_core.RscIconFile.get('tab/tab-add')
        )
        ...
        self._tab_choose_button._set_icon_file_path_(
            gui_core.RscIconFile.get('tab/tab-choose')
        )

        self._tab_menu_is_enable = False
        self._tab_menu_button = _gui_qt_wgt_button.QtIconMenuButton(self)
        self._tab_menu_button.hide()
        self._tab_menu_button._set_icon_file_path_(
            gui_core.RscIconFile.get('tab/tab-menu-v')
        )
        ...
        self._layer_stack = _gui_qt_wgt_layer_stack.QtLayerStack(self)
    ...
    def paintEvent(self, event):
        ...
        painter._draw_tab_buttons_by_rects_(
            ...
            virtual_items=self._tab_item_stack.get_items(),
    ...
    def _add_widget_(self, widget, *args, **kwargs):
        ...
        if 'name' in kwargs:
            tab_item.set_name_text(kwargs['name'])
    ...
    def _set_current_index_(self, index):
        self._item_index_pressed = None
        self._layer_stack._set_current_index_(index)
        self._refresh_widget_all_()
    ...
    def _set_item_current_by_name_text_(self, text):
        index = self._tab_item_stack.get_index_by_name_text(
            text
        )
    ...
    def _compute_press_index_loc_(self, p):
        if self._tab_bar_rect.contains(p):
            for i_index, i_tab_item in enumerate(self._tab_item_stack.get_items()):
    ...
    def _get_current_name_text_(self):
        return self._tab_item_stack.get_name_text_at(

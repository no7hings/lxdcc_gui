...
from lxutil_gui.qt.widgets import _gui_qt_wgt_utility, _gui_qt_wgt_entry_base, _gui_qt_wgt_item_for_tree, _gui_qt_wgt_view_for_tree
...
class _QtNGConnection(
    ...
    def _refresh_widget_draw_geometry_(self, rect):
        ...
        x_, y_ = p.x(), p.y()
        self._ng_draw_connection_coord_arrow = [
            gui_core.Ellipse2dMtd.get_coord_at_angle_(center=(x_, y_), radius=r_, angle=90+a),
            gui_core.Ellipse2dMtd.get_coord_at_angle_(center=(x_, y_), radius=r_, angle=210+a),
            gui_core.Ellipse2dMtd.get_coord_at_angle_(center=(x_, y_), radius=r_, angle=330+a),
            gui_core.Ellipse2dMtd.get_coord_at_angle_(center=(x_, y_), radius=r_, angle=90+a)
        ]
...
class _QtNGTreeNode(
    _gui_qt_wgt_item_for_tree.QtTreeWidgetItem
):
    def __init__(self, *args, **kwargs):
...
class _QtNGTree(
    _gui_qt_wgt_view_for_tree.QtTreeWidget,
    AbsQtNGUniverseDef
):
    ...
    def _set_ng_node_add_0_(self, obj):
        ...
        if icon_file_path:
            ...
            item._set_icon_file_path_(
                gui_core.RscIconFile.get('obj/group')
            )
...
class _QtNGImageGraph(_QtNGGraph):
    ...
    def _get_ng_graph_image_rect_args_(self):
        ng_nodes = self._ng_graph_nodes
        xs_0, ys_0 = [i.x() for i in ng_nodes], [i.y() for i in ng_nodes]
    ...
    def _set_ng_graph_image_save_to_(self, file_path):
        ng_nodes = self._ng_graph_nodes
        scale = 10
        m = 48

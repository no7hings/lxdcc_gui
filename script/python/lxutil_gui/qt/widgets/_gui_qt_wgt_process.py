...
class _QtProcessingThread(QtCore.QThread):
    ...
    def filter_logging(self, text):
        process_start = bsc_core.LogMtd.filter_process_start(text)
        if process_start:
            _count = process_start[-1]
            self.start_processing(_count)

        process = bsc_core.LogMtd.filter_process(text)
        if process:
            self.update_processing()

        result = bsc_core.LogMtd.filter_result(text)
        if result:
    ...
    def run(self):
        self.update_logging(
            bsc_core.LogMtd.get_method_result(
                self.KEY, 'is started'
            )
        )
        ...
        try:
            ...
            import traceback
            self.update_logging(
                bsc_core.LogMtd.get_method_error(
                    self.KEY, 'is failed'
                )
            )
            self.update_logging(
...
class QtProcessingBar(QtWidgets.QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        ) = gui_qt_abstract.AbsQtStatusBaseDef._get_sub_process_status_rgba_args__(
            self.Status.Waiting
        )

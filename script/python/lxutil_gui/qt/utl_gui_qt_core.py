...
class QtProgressingChartDrawMtd(object):
    @classmethod
    def _get_basic_data_(cls, rect, index, percent, percent_range, label, tape_w=8, spacing=8):
...
class QtPainter(QtGui.QPainter):
    ...
    def _set_empty_draw_by_rect_(self, rect, icon_name=None):
        x, y = rect.x(), rect.y()
        w, h = rect.width(), rect.height()
        r = min(w, h)
        ...
        if file_path:
            if os.path.isfile(file_path):
                if file_path.endswith('.svg'):
                    ...
                    self._draw_image_by_rect_(rect, file_path, offset)

    def _draw_image_by_rect_(self, rect, file_path, offset=0, is_hovered=False):
        rect_ = QtCore.QRect(
            rect.x()+offset, rect.y()+offset,
            rect.width()-offset, rect.height()-offset
        )
        ...
        new_pixmap = QtGui.QPixmap(new_image)
        self.drawPixmap(
            rect_,
            new_pixmap
        )
        if is_hovered:
            image = QtGui.QImage(
                rect_.width(), rect_.height(), QtGui.QImage.Format_ARGB32
            )
            image.fill(QtCore.Qt.black)
            painter = QtGui.QPainter(image)
            painter.drawPixmap(QtCore.QRect(0, 0, rect_.width(), rect_.height()), new_pixmap)
            painter.end()
            #
            mask_pixmap = QtGui.QPixmap(image)
            mask_bitmap = QtGui.QPixmap(image)
            mask = mask_bitmap.createMaskFromColor(QtCore.Qt.black)
            mask_pixmap.fill(QtGui.QColor(255, 127, 63, 127))
            mask_pixmap.setMask(mask)
            rect__ = QtCore.QRect(
                rect.x(), rect.y(),
    ...
    def _draw_svg_by_rect_(self, rect, file_path, offset=0, is_hovered=False):
        rect_ = QtCore.QRect(
            rect.x()+offset, rect.y()+offset,
            rect.width()-offset, rect.height()-offset
        )
        ...
        if is_hovered is True:
            ...
            svg_render.render(painter, QtCore.QRectF(0, 0, rect_.width(), rect_.height()))
            painter.end()
            #
            mask_pixmap = QtGui.QPixmap(mask_image)
            mask_bitmap = QtGui.QPixmap(mask_image)
            mask = mask_bitmap.createMaskFromColor(QtCore.Qt.black)
            mask_pixmap.fill(QtGui.QColor(255, 127, 63, 127))
            mask_pixmap.setMask(mask)
            rect__ = QtCore.QRect(
    ...
    def _draw_icon_with_name_text_by_rect_(self, rect, text, border_color=None, background_color=None, font_color=None, offset=0, border_radius=0, border_width=1, is_hovered=False, is_enable=True):
        if offset != 0:
            rect_offset = QtCore.QRect(
                rect.x()+offset, rect.y()+offset,
                rect.width()-offset, rect.height()-offset
    ...
    def _set_tab_button_draw_(self, rect, name_text, icon_name_text=None, border_width=1, offset=0, is_hovered=False, is_selected=False):
        a = 255
        if is_selected:
        ...
        if is_selected is True:
            ...
            frame_coords = [
                (frm_x, frm_y+frm_h), (frm_x+frm_h, frm_y), (frm_x+frm_w-frm_h, frm_y), (frm_x+frm_w, frm_y+frm_h), (frm_x, frm_y+frm_h)
            ]
        #
        ...
        #     l_w, l_h = 48, 4
        #     l_b_r = l_h/2
        #     l_x, l_y = frm_x+(frm_w-l_w)/2, frm_y+frm_h-2
        #     line_rect = QtCore.QRect(
        #         l_x, l_y, l_w, l_h
        #     )
        #     self._set_border_color_(QtBorderColors.Transparent)
        #     self._set_background_color_(QtBackgroundColors.Selected)
        #     self.drawRoundedRect(line_rect, l_b_r, l_b_r, QtCore.Qt.AbsoluteSize)
        #
        i_f_x, i_f_y = frm_x+r, frm_y
        i_f_w, i_f_h = frm_h, frm_h
        i_w, i_h = 12, 12
        t_x = i_f_x+s
        t_w = w
        if icon_name_text is not None:
            ...
            self._draw_icon_with_name_text_by_rect_(
                rect=icon_rect,
                text=icon_name_text,
                border_radius=i_h/2
            )
